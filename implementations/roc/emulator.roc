...
asl : PartialByteOp
asl = shiftRot (\_c,v -> { c: Num.bitwiseAnd v 0x80, v: Num.shiftLeftBy v 1 })

lsr : PartialByteOp
lsr = shiftRot (\_c,v -> { c: Num.bitwiseAnd v 0x01, v: Num.shiftRightZfBy v 1 })

rol : PartialByteOp
rol = shiftRot (\c,v -> { c: Num.bitwiseAnd v 0x80, v: Num.bitwiseOr (Num.shiftLeftBy v 1) (if c then 0x01 else 0x00) })

ror : PartialByteOp
ror = shiftRot (\c,v -> { c: Num.bitwiseAnd v 0x01, v: Num.bitwiseOr (Num.shiftRightZfBy v 1) (if c then 0x80 else 0x00) })
...
step = \cpu ->
    ...
    when op is
        ...
        0x18 -> always (setFlag fetchCpu carry Bool.false)
        0x38 -> always (setFlag fetchCpu carry Bool.true)
        0x58 -> always (setFlag fetchCpu interruptEnable Bool.false)
        0x78 -> always (setFlag fetchCpu interruptEnable Bool.true)
        0xb8 -> always (setFlag fetchCpu overflow Bool.false)
        0xd8 -> always (setFlag fetchCpu decimal Bool.false)
        0xf8 -> always (setFlag fetchCpu decimal Bool.true)
